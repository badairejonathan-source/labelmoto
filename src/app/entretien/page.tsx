...
export default function EntretienPage() {
  ...
  const imageUrl = useMemo(() => {
    ...
    return "/images/motard-entretien-page.png";
  }, [article]);
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
        <div className="max-w-6xl mx-auto">
          <nav className="flex items-center gap-2 text-muted-foreground text-[10px] font-black uppercase tracking-widest mb-8">
            <Link href="/" className="hover:text-brand transition-colors flex items-center gap-1"><Home className="h-3 w-3" /><span>Accueil</span></Link>
            <ChevronRight className="h-3 w-3" /><span className="text-foreground">Entretien & Révisions</span>
          </nav>
          <div className="mb-12">
            <h1 className="text-4xl md:text-6xl font-black leading-[1.1] tracking-tighter text-foreground mb-4 uppercase">Catalogue des Fiches Techniques</h1>
            <p className="text-xl text-muted-foreground font-bold max-w-3xl">Sélectionnez une marque pour accéder aux guides d'entretien officiels et aux spécifications techniques.</p>
          </div>
              ...
              <div className="pt-24 border-t border-border/50">
                {isArticleLoading ? (
                  ...
                  <article className="prose prose-neutral dark:prose-invert max-w-none">
                    ...
                    <div className="relative w-full aspect-[4/3] md:aspect-[2/1] rounded-3xl overflow-hidden mb-12 shadow-xl border-4 border-white bg-muted"><Image src={imageUrl} alt={article.display_title || article.title} fill className="object-cover" /></div>
                    <div className="mb-12">
                      {article.intro && Array.isArray(article.intro) && article.intro.map((p: string, i: number) => (<p key={i} className="text-xl leading-relaxed text-foreground font-black mb-4">{p}</p>))}
