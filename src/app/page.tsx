            ...
            <main className="py-12 px-4 sm:px-6 lg:px-8">
              <div className="max-w-6xl mx-auto">
                <div className="relative rounded-2xl border-2 border-brand bg-black mb-24 md:mb-32 overflow-visible shadow-2xl">
                     <Image src={hero.src} alt="Motorcycle Urban" fill className="object-cover z-0 opacity-40 rounded-2xl" priority data-ai-hint={hero.hint} />
                    <div className="relative z-10 flex flex-col md:flex-row items-center gap-4 md:gap-6 text-white p-6 md:p-12 md:pb-20">
                        <div className="md:w-3/5 text-center md:text-left relative z-20">
                            <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight mb-2 md:mb-4 uppercase leading-[0.95]" style={{ textShadow: '0 3px 6px rgba(0,0,0,0.5)' }}>Du A2 au motard expérimenté : trouvez les professionnels les plus proches en quelques clics</h1>
                            <p className="text-sm md:text-lg max-w-3xl mx-auto md:mx-0 mb-4 md:mb-6 text-gray-200 font-medium" style={{ textShadow: '0 2px 4px rgba(0,0,0,0.5)' }}>Fini les dizaines d'onglets ouverts. Label Moto regroupe tout l’univers deux-roues au même endroit pour vous laisser plus de temps sur la route.</p>
                        </div>
                         <div className="w-full md:w-2/5 flex justify-center md:justify-end relative z-10">
                             <div className="relative transform translate-y-6 md:translate-y-24 lg:translate-y-28 group">
                                <Link href="/map" className="block transform hover:scale-105 transition-transform duration-300">
                                    <div className="relative w-48 h-48 md:w-72 md:h-72"><Image src={hero.mapPreview.src} alt="Aperçu de la carte" fill className="rounded-xl border-4 border-white shadow-2xl object-cover" data-ai-hint={hero.mapPreview.hint} /></div>
                                </Link>
                                <div className="absolute -left-8 md:-left-20 top-1/2 -translate-y-1/2 z-50">
                                    <Link href="/map"><Button size="lg" className="bg-brand hover:bg-brand/90 text-brand-foreground font-black uppercase text-[10px] md:text-base px-4 md:px-8 py-3 md:py-6 rounded-full shadow-2xl border-2 md:border-4 border-white whitespace-nowrap transition-transform hover:scale-105 tracking-widest">Explorer la carte</Button></Link>
                                </div>
                             </div>
                        </div>
                    </div>
                ...
                <section className="mt-20 md:mt-32">
                    <div className="bg-muted/50 rounded-3xl p-8 border border-border/50 backdrop-blur-sm">
                        <h2 className="text-3xl md:text-5xl font-black text-foreground mb-8 text-center uppercase tracking-tighter">Pourquoi choisir LABEL MOTO</h2>
                        <ul className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
                            <li className="flex items-start gap-4"><CheckCircle className="h-6 w-6 text-brand shrink-0 mt-1" /><div><h3 className="font-black uppercase tracking-tight text-lg mb-1">Gain de temps</h3><p className="text-sm text-muted-foreground leading-relaxed">Recherche simplifiée, centralisez vos besoins. Localisation, type de moto, expertise spécifique : <span className="font-bold text-foreground">Label Moto fait le tri pour vous.</span></p></div></li>
                            <li className="flex items-start gap-4"><CheckCircle className="h-6 w-6 text-brand shrink-0 mt-1" /><div><h3 className="font-black uppercase tracking-tight text-lg mb-1">Données vérifiées</h3><p className="text-sm text-muted-foreground leading-relaxed">Toutes les données des concessions (occasion et neuve) sont vérifiées et <span className="font-bold text-foreground">régulièrement actualisées</span> pour vous garantir des informations correctes.</p></div></li>
                            <li className="flex items-start gap-4"><CheckCircle className="h-6 w-6 text-brand shrink-0 mt-1" /><div><h3 className="font-black uppercase tracking-tight text-lg mb-1">Transparence totale</h3><p className="text-sm text-muted-foreground leading-relaxed">Découvrez de <span className="font-bold text-foreground">vrais avis</span> partagés par une communauté de motards, sans filtres ni publicité déguisée, pour faire le bon choix.</p></div></li>
                             <li className="flex items-start gap-4"><CheckCircle className="h-6 w-6 text-brand shrink-0 mt-1" /><div><h3 className="font-black uppercase tracking-tight text-lg mb-1">Expertise technique</h3><p className="text-sm text-muted-foreground leading-relaxed">Accédez à nos guides et fiches techniques pour maîtriser votre sujet et <span className="font-bold text-foreground">identifier le professionnel idéal</span> pour votre machine.</p></div></li>
                        </ul>
                    </div>
                </section>
