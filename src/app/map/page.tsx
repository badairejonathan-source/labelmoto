...
function MapPageComponent() {
  ...
  // PIPELINE DE DONNÉES AVEC DIAGNOSTIC
  useEffect(() => {
    const fetchFreshData = async () => {
      ...
      const collections = [
        { id: 'concessions', debug: 'MAP_DEBUG_1' },
        { id: 'associations', debug: 'MAP_DEBUG_2' },
        { id: 'relais', debug: 'MAP_DEBUG_3' },
        { id: 'creators', debug: 'MAP_DEBUG_CREATORS' }
      ];

      const allPoints: MapPoint[] = [];
      const seenIds = new Set<string>();
      let rejectedCoords = 0;
      let rejectedType = 0;

      for (const col of collections) {
        try {
          const snap = await getDocs(query(collection(firestore, col.id), limit(500)));
          ...
          snap.docs.forEach(docSnap => {
            ...
            // MAP_DEBUG_4 : Validation des coordonnées
            ...
            // Déduction de la section app
            const appSection = data.appSection || (col.id === 'associations' ? 'association' : (col.id === 'relais' ? 'relais' : (col.id === 'creators' ? 'creator' : 'shopping')));
            
            // MAP_DEBUG_5 : Validation du type (optionnel car on force le défaut)
            if (!appSection) {
              rejectedType++;
              return;
            }
            ...
            allPoints.push({
              ...
              category: data.category || (col.id === 'associations' ? 'association' : (col.id === 'relais' ? 'relais' : 'concession')),
          ...
          console.warn(`[MAP] Erreur collection ${col.id}:`, e);
        }
      }

      console.log(`[MAP_DEBUG_4] Rejetés pour coordonnées invalides: ${rejectedCoords}`);
      console.log(`[MAP_DEBUG_5] Rejetés pour type/catégorie invalide: ${rejectedType}`);
