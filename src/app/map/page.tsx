...
function MapPageComponent() {
  ...
  const dealershipsToDisplay = useMemo(() => {
    ...
    if (mapBoundsStr) { const [minLng, minLat, maxLng, maxLat] = mapBoundsStr.split(',').map(Number); results = results.filter(d => d.latitude && d.longitude && d.latitude >= minLat && d.latitude <= maxLat && d.longitude >= minLng && d.longitude <= maxLng); }
    if (selectionSource === 'marker' && selectedDealershipId) { const selected = results.find(d => d.id === selectedDealershipId); if (selected && selected.latitude && selected.longitude) { const others = results.filter(d => d.id !== selectedDealershipId); others.sort((a, b) => getDistanceSq(sortingAnchor, a) - getDistanceSq(sortingAnchor, b)); return [selected, ...others].slice(0, 30); } }
  ...
  const listContent = (
    <div className="space-y-3 pb-20">
      {!hasUserInitiatedAction ? (
        ...
        <>{dealershipsToDisplay.map((dealer, index) => (<React.Fragment key={dealer.id}><div onMouseEnter={() => setHoveredDealershipId(dealer.id)} onMouseLeave={() => setHoveredDealershipId(null)}><DealershipCard dealership={dealer} onClick={() => handleCardClick(dealer)} className={cn(dealer.id === selectedDealershipId && "ring-2 ring-brand")} /></div>{(index + 1) % 4 === 0 && (<div className="my-3"><AdCard article={ads[Math.floor(index / 4) % ads.length]} /></div>)}</React.Fragment>))}{dealershipsToDisplay.length === 0 && (<div className="text-center py-20 text-muted-foreground px-6"><div className="w-12 h-12 bg-muted/30 rounded-full flex items-center justify-center mx-auto mb-4"><SearchIcon className="h-6 w-6 opacity-20" /></div><p className="font-bold uppercase text-[10px] tracking-widest">Aucun établissement dans cette zone.</p><p className="text-[9px] mt-2 opacity-60">Essayez de dézoomer ou de changer de ville.</p></div>)}</>
      )}
    </div>
  );
