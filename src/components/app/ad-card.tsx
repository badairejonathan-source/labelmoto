...
const AdCard: React.FC<AdCardProps> = ({ article }) => {
  ...
  return (
    <Link href={`/info/${article.id}`} className="group block w-full">
      <Card className="w-full ml-auto overflow-hidden transition-all duration-500 ease-in-out border border-brand/20 border-l-4 border-l-brand hover:shadow-lg bg-gradient-to-r from-brand/[0.02] to-background shadow-sm min-h-[120px] md:min-h-[160px] flex items-stretch relative">
        ...
        {/* Section Image - Now taking full height explicitly */}
        <div className="relative w-32 sm:w-48 md:w-64 flex-shrink-0 overflow-hidden bg-muted border-r border-brand/5">
          <Image
            src={imageUrl}
            alt={article.title}
            fill
          ...
          <div className="absolute top-3 left-3 flex items-center gap-1.5 text-[9px] md:text-[11px] font-black text-white bg-brand rounded-full px-2.5 py-1 uppercase tracking-widest shadow-lg z-20">
        ...
        {/* Zone Action */}
        <div className="hidden md:flex flex-shrink-0 w-32 flex-col justify-center items-center p-4 bg-brand/[0.01] border-l border-brand/5 z-10">
        ...
        <div className="md:hidden flex items-center pr-4 bg-brand/[0.01] z-10">
