...
export default function CookieConsent() {
  ...
  // Fonction pour injecter Analytics après le rendu critique
  ...
  useEffect(() => {
    ...
    if (!consent) {
      // Premier passage : on montre la bannière après un court délai
      const timer = setTimeout(() => setShowBanner(true), 1500);
      return () => clearTimeout(timer);
    } else if (consent === 'accepted') {
      // Déjà accepté : on charge Analytics après la première interaction utilisateur
      // Cela évite de charger les tags pendant le rendu initial (TBT optimization)
      ...
      // Sécurité : charger après 5 secondes même sans interaction
  ...
  return (
    <>
      {/* Chargement différé du script Analytics */}
      {loadAnalytics && gaId && (
